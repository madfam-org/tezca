...
## #222 follow-up (2026-09-23) — what is live, what to run now

**Observed (read-only `GET`s against `api.tezca.mx`, 2026-09-23).**

- `lfpdppp`: the first re-index DID run. Arts. 1–64 now serve the substantive
  2025 text (2 = definiciones, 8 = datos sensibles «expreso y por escrito»,
  9 = excepciones al consentimiento). But the listing has **109** entries:
  `T-1`…`T-10`, 10 orphan word ids from the pre-#218 index (`Primero` …
  `Décimo`), and 25 `N-dupK` docs (`1-dup1`, `10-dup1`…`10-dup11`, …) that are
  transitorio text under substantive-looking ids. Law `status` is `unknown`.
- Root cause of the `-dupN` docs: the production XML was generated by the
  Feb-2026 parser, which serialised every ordinal line of the TRANSITORIOS
  sections as `<num>Artículo N</num>` nodes (ORDINAL_TO_NUMBER, first word only,
  so «Décimo Primero»…«Décimo Noveno» all became `10`). #218's indexer kept them
  apart only by suffixing. The DOF 14-11-2025 reform decree's transitorios
  (a second «Primero»…«Cuarto» block) existed ONLY as such dups — the current
  parser dropped repeated numbers.
- **Other laws are still collided — never re-indexed since #218.** Sampled:
  `amparo` arts. 1–10, `cff` 1–6 and 8–10, `lft` 1–10 and 20, `lisr` 1–4, 6 and 8–10 serve
  transitorio text («Primero. El presente Decreto…») in place of the article.
  Expect every federal law ingested with the Feb-2026 parser to be affected.
- `reg_reg_lfpdppp`: ingested (149 docs = 144 articles + `T-1`…`T-5`, grade A).
  Its vigencia is legally open; see the `notes` on its registry entry.

**What #222 changed in code.** Legacy-XML-aware ids (`apps/api/transitorios.py`:
ordinal-headed text under a duplicate id becomes `T-N`, a numbering restart
opens decree block `T-R2-N`, wrapped-line fragments merge back, same-provision
copies collapse); in-place re-index prunes stale docs; the parser keeps every
decree's transitorios (`trans-rK-N`); the API returns `kind`; new
`dataops.sync_law_status` promotes `unknown` statuses from the registry.

**Run, in order (operator; production writes).** Each command runs the
registered Celery task **synchronously** in a Django shell on the live
`tezca-worker` pod (same image, env and NetworkPolicy identity as the worker)
and prints the task's return dict. Why not `enclii jobs …`: see
[How to invoke the `dataops.*` tasks](#how-to-invoke-the-dataops-tasks-corrected-2026-09-23).

```bash
# 0. Read-only pre-flight: imports resolve, DB reachable, current status.
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.api.models import Law; l = Law.objects.get(official_id=\"lfpdppp\"); print(\"status=\" + str(l.status), \"newest=\" + str(l.versions.first().publication_date))"'
#    expect: status=unknown newest=2025-03-20

# 1. Status (DB): preview, then apply. Only unknown/blank rows change.
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import sync_law_status; print(sync_law_status(law_id=\"lfpdppp\", dry_run=True))"'
#    expect: {'status': 'completed', 'dry_run': True, 'updated': [{'law_id': 'lfpdppp', 'to': 'vigente'}], ...}
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import sync_law_status; print(sync_law_status(law_id=\"lfpdppp\"))"'
#    expect: same dict with 'dry_run': False

# 2. Re-index LFPDPPP from the existing XML (no re-parse needed): preview, then run.
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import reindex_law; print(reindex_law(law_id=\"lfpdppp\", dry_run=True))"'
#    expect: 'status': 'completed' and output 'Dry run: Would index 88 articles for lfpdppp'
#    (88 = 1–64, T-1…T-20, T-R2-1…T-R2-4). Any other count: STOP, do not run the real pass.
# REAL RUN: omit dry_run entirely. reindex_law reads it with bool(), so any
# non-empty string ("false", "0") would silently mean a dry run.
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import reindex_law; print(reindex_law(law_id=\"lfpdppp\"))"'
#    expect: output contains 'Connected to Elasticsearch at …' AND
#    'Pruned 35 stale article docs for lfpdppp' (10 word ids + 25 -dupN ids).
#    'status': 'completed' alone is NOT proof: when ES is unreachable index_laws
#    prints 'Elasticsearch offline' to stderr and returns without raising.

# 3. RLFPDPPP is already ingested; re-index only so its docs carry kind/prune.
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import reindex_law; print(reindex_law(law_id=\"reg_reg_lfpdppp\", dry_run=True))"'
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import reindex_law; print(reindex_law(law_id=\"reg_reg_lfpdppp\"))"'
```

Corpus-wide (the other collided laws): run `dataops.sync_law_status` without
`law_id` (dry run first), then either `dataops.reindex_law` per affected law or,
from a break-glass pod shell, `python manage.py index_laws --all --reindex`
(§1B — new versioned index + alias swap; never `--reindex --law-id`).

**Acceptance.**

```bash
curl -s "https://api.tezca.mx/api/v1/laws/lfpdppp/articles/?page_size=500" | python3 -c "
import sys, json
d = json.load(sys.stdin); m = {a['article_id']: a for a in d['articles']}
for k in ('2', '7', '8', '9'): print(k, m[k]['kind'], '->', ' '.join(m[k]['text'].split())[:70])
print('transitorios:', sorted(k for k in m if k.startswith('T-')))
bad = [k for k in m if 'dup' in k or (k[:1].isalpha() and not k.startswith('T-'))]
print('total', d['total'], '| stale ids:', bad or 'none')"
curl -s https://api.tezca.mx/api/v1/laws/lfpdppp/ | python3 -c "import sys,json; print('status', json.load(sys.stdin)['status'])"
curl -s https://api.tezca.mx/api/v1/laws/reg_reg_lfpdppp/ | python3 -c "import sys,json; print('reg articles', json.load(sys.stdin)['articles'])"
```

Pass = `2` definiciones, `7` consentimiento, `8` «expreso y por escrito»,
`9` «no estará obligado a recabar el consentimiento», all `kind=articulo`;
`T-1`…`T-20` and `T-R2-1`…`T-R2-4` present with `kind=transitorio`; total 88;
no stale ids (all 35 gone: `Primero`…`Décimo` and every `-dupN`); status
`vigente`; `reg_reg_lfpdppp` articles > 0.

---

> **⚠️ INVOCATION CORRECTION (2026-08-26, revised 2026-09-23).** Earlier drafts
> of this runbook used `enclii exec tezca-api -- python apps/manage.py …`.
> **`enclii exec` is NOT a real subcommand**, and neither is `enclii jobs run`
> (the CLI, v1.0.0-alpha.9, only has `enclii jobs run-once`). `run-once` is not a
> working path for these tasks either — see
> [How to invoke the `dataops.*` tasks](#how-to-invoke-the-dataops-tasks-corrected-2026-09-23).
> The working invocation runs the registered task synchronously on the live
> worker pod, e.g. the LFPDPPP re-index dry run:
>
> ```bash
> ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import reindex_law; print(reindex_law(law_id=\"lfpdppp\", dry_run=True))"'
> ```
>
> `dataops.reindex_law` (in `apps/scraper/scheduling/tasks.py`) wraps
> `index_laws --law-id <id>` — the in-place upsert path, and it **refuses** the
> corpus-dropping `--reindex`/`--all` variants. The raw `enclii exec … python
> manage.py …` lines in §1–§2 are kept as the reference for WHAT runs; execute
> them through the same `kubectl … exec deploy/tezca-worker -c worker --` wrapper
> (manage.py is at `/app/manage.py`, the working directory).
>
...
> **Stale-doc note (corrected by #222).** An upsert alone cannot delete an id
> the new output no longer emits, and this note used to claim no orphans would
> appear. They did: the first re-index left the pre-#218 word ids
> (`lfpdppp-Primero` … `lfpdppp-Décimo`) and 25 `-dupN` docs behind (109
> entries). Since #222 an in-place run **prunes** the law's article docs it did
> not rewrite (exact `term` on `law_id`, skipped if the run wrote nothing);
> pass `--no-prune` to opt out.
...
```bash
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import ingest_law; print(ingest_law(law_id=\"reg_reg_lfpdppp\"))"'
```
...
the XML, then index deliberately with `reindex_law(law_id="reg_reg_lfpdppp")`
(or `index_laws --include-quarantined`). Pass `index=False` (a Python bool, in
the same shell wrapper) to persist only: `ingest_law(law_id="reg_reg_lfpdppp", index=False)`.
...
**Enclii-first remains the rule** (see `AGENTS.md`), but Enclii has no working
adapter for these tasks today (next section), so the registered task runs on the
live worker pod through the documented break-glass path:

```sh
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import ingest_law; print(ingest_law(law_id=\"reg_reg_lfpdppp\"))"'
```

The `tezca-worker` pods carry `app.kubernetes.io/part-of: tezca` and therefore
inherit the NetworkPolicy allowances below.

> **Enclii adapter gap (recorded 2026-08-27, extended 2026-09-23).** There is no
> Enclii adapter for an interactive shell in a running pod — `enclii exec` does
> not exist — and `enclii jobs run-once` cannot run these tasks (see
> [How to invoke the `dataops.*` tasks](#how-to-invoke-the-dataops-tasks-corrected-2026-09-23)).
> Staging a file into a pod (the pre-placed PDF above) likewise has no Enclii path.
...
| `tezca-worker` | `…/tezca/api` (same image) | `celery -A apps.indigo worker` | Yes — target of the `dataops.*` shell invocations |
...
## How to invoke the `dataops.*` tasks (corrected 2026-09-23)

Every earlier `enclii jobs run dataops.<task> -- key=val --service tezca-worker
--env production` line in this repo describes a CLI shape that does not exist.
What was checked (read-only, 2026-09-23):

- **`enclii jobs run` does not exist.** `enclii jobs --help` (v1.0.0-alpha.9)
  lists `create`, `delete`, `get`, `list`, `logs`, `run-once`. `run-once` takes
  `--name --command --service-id --project [--image] [--timeout]`; the command
  runs as `/bin/sh -c <command>` in a new Kubernetes Job.
- **`run-once` job pods cannot reach Postgres or Redis.** The Job's pods carry
  only `app.kubernetes.io/managed-by: enclii` and `enclii.dev/one-off-job-id`
  (switchyard `buildOneOffJob`), not `app.kubernetes.io/part-of: tezca`, so none
  of the tezca NetworkPolicies (`allow-data-egress`, `allow-intra-namespace`)
  select them, and the `data` namespace refuses the connection. Proven: the
  one-off job `job-reindex-lfdpppp-dry2-…` (2026-08-27, command `python manage.py
  shell -c "…reindex_law(law_id=\"lfpdppp\", dry_run=True)…"`) shows
  **Completed**, but its log is `{'status': 'error', … 'error': 'connection to
  server at "postgres.data.svc.cluster.local" … Connection refused'}`. The task
  catches the exception and returns a dict, so the job exits 0 — a green job
  that did nothing. Three sibling jobs failed earlier on `apps/manage.py`.
- **Queueing with `celery call` from a one-off job has the same problem**: the
  broker (`tezca-redis`) only admits `part-of: tezca` pods.

**Working invocation** — the registered task, called synchronously in a Django
shell on the live worker pod (same image, env and pod identity as the worker):

```bash
ssh ssh.madfam.io 'sudo -n kubectl -n tezca exec deploy/tezca-worker -c worker -- python manage.py shell -c "from apps.scraper.scheduling.tasks import <task>; print(<task>(<kwargs>))"'
```

- Pass kwargs as Python literals: `law_id=\"lfpdppp\"` (escaped double quotes
  inside the single-quoted ssh string), `dry_run=True`.
- **`reindex_law` reads `dry_run` with `bool()`.** For the real run, omit
  `dry_run`; never pass it as a string. (`sync_law_status` uses `_truthy()` and
  accepts either, but omit it for the real run too for symmetry.)
- The task writes its `AcquisitionLog` row exactly as a Celery-dispatched run
  would; the printed dict is the task's return value.
- Verified read-only on 2026-09-23 with this wrapper: the imports resolve
  (`dataops.reindex_law`, `dataops.sync_law_status`), the worker reaches Postgres
  (`lfpdppp` status=`unknown`, newest version 2025-03-20) and Elasticsearch
  answers a ping.

Enclii follow-up (adapter gap): one-off job pods should inherit the target
Deployment's pod-template labels (at least `app.kubernetes.io/part-of`) so
`enclii jobs run-once` can become the sanctioned path again.
