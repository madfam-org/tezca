...
- `lfpdppp`: the first re-index DID run. Arts. 1–64 now serve the substantive
  ...
  `Décimo`), and 24 `N-dupK` docs (`1-dup1`, `10-dup1`…`10-dup11`, …) that are
...
**Run, in order (operator; production writes).**
...
enclii jobs run dataops.sync_law_status -- law_id=lfpdppp               --service tezca-worker --env production
# 2. Re-index LFPDPPP from the existing XML (no re-parse needed): preview, then run.
#    Expect 88 articles: 1–64, T-1…T-20, T-R2-1…T-R2-4; the run prunes the 10 word ids + 24 dups.
...
enclii jobs run dataops.reindex_law -- law_id=reg_reg_lfpdppp --service tezca-worker --env production
```
...
no stale ids; status `vigente`; `reg_reg_lfpdppp` articles > 0.
...
> (`lfpdppp-Primero` … `lfpdppp-Décimo`) and 24 `-dupN` docs behind (109
